#!/usr/bin/env python3
"""Generate audited visible-only CIPHER_MAP_V1 competition rows.

The mapping search is run without the stored answer. The stored answer is used
only after prediction to decide whether the row is safe to train.
"""
...
from generators.trace_transform import build_cipher_trace
    ...
    with open(train_csv) as f:
        for row in csv.DictReader(f):
            ...
            if "{" in answer or "}" in answer:
                assistant += f"The final answer is: {answer}"
            else:
            ...
            rows.append({
                ...
                "mode": "cipher_map_visible",
                "generator": "gen_transform_cipher_visible_rows",
...
def main() -> None:
    ...
    parser.add_argument("--out", default="data/transformation/pool/competition/cipher_map_visible.jsonl")
    parser.add_argument("--summary", default="data/transformation/pool/competition/cipher_map_visible.summary.json")
